...
| Parameter | Type | Required | Expected Format | Description |
| :--- | :--- | :--- | :--- | :--- |
| Content-Type | String | **Yes** | application/json or multipart/form-data | Dictates the serialisation format of the request body (See Section 4.6). |
| Authorization | String | **Yes** | Bearer <token> | Token-based security credentials matching the pre-shared API key. |


> **Request Body Parameters (JSON Payload - application/json)**

| Parameter | Type | Required | Size Constraints | Description |
| :--- | :--- | :--- | :--- | :--- |
| prompt | String | No | 0 ≤ 𝐿 ≤ 16,384 bytes | The text query to be processed by the target AI agent. Optional if an image payload is provided. |
| session_id | String | No | 0 ≤ 𝐿 ≤ 128 bytes | Unique ID for conversation thread tracking. |
| target_agent_id | String | No | 0 ≤ 𝐿 ≤ 64 bytes | Direct request to target a specific agent (e.g. agent-2). |
| image | Object | No | See constraints below | Optional multimodal input containing image data. |
| image.data | String | Yes (if image present) | 1 ≤ 𝐿 ≤ 5 MB (Base64) | Base64-encoded representation of the binary image data. |
| image.mime_type | String | Yes (if image present) | image/png, image/jpeg | The media format type of the uploaded image. |
